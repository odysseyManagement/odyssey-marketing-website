...
const ServicesV1 = ({ hasTitle, sectionClass }: DataType) => {
  ...
  // cycle through services every second if not hovering
  useEffect(() => {
    if (isHovering) return; // pause auto cycle on hover

    const interval = setInterval(() => {
      setActiveServiceId((prevId) => {
        const currentIndex = ServicesV1Data.findIndex((s) => s.id === prevId);
        const nextIndex = (currentIndex + 1) % ServicesV1Data.length;
        return ServicesV1Data[nextIndex].id;
      });
    }, 4000);
            ...
            <div className="row">
              <div className="col-xl-6 offset-xl-3 col-lg-8 offset-lg-2 text-center">
                <div className="site-heading">
                  <h4 className="sub-title">What We Do For Creators</h4>
                  <h2 className="title split-text">
                    <SplitText
                      ...
                      rootMargin="-50px"
                    >
                      Services That Build Your Brand & Grow Your Income
                    </SplitText>
                  </h2>
                </div>
              </div>
